...
User.prototype.validate = function(){
    return new Promise(async (resolve, reject) => {
        ...
        if(this.data.password.length > 50){ this.errors.push("password cannot exceeds 50 characters")}
        if(this.data.username.length > 0 && this.data.password.length < 3){ this.errors.push("Username must be at least 3 characters")}
        if(this.data.username.length > 30){ this.errors.push("password cannot exceeds 30 characters")}
   ...
   return new Promise((resolve, reject)=>{
    ...
    usersCollection.findOne({username: this.data.username}).then((attemptedUser)=>{
        if(attemptedUser && bcrypt.compareSync(this.data.password, attemptedUser.password)){
            ...
            reject("Login Failed .... Please enter correct UserNAme and Password")
        }
    }).catch(function(){
        reject("Please try again latter")
    })
   })
}
...
User.prototype.register = function(){
    return new Promise(async (resolve, reject) => {
        ...
        //step2: only if there are no validation errors then save a userdata into a database
